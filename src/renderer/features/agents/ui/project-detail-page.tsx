...
function ProjectWorktreeSettings({ projectId }: { projectId: string }) {
  ...
  const saveStartCommandsMutation = trpc.projects.updateStartCommands.useMutation({
    ...
    onError: (err) => {
      console.error("Failed to save start commands:", err)
    },
  })
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="space-y-3">
          <div>
            ...
            <textarea
              ...
              className="w-full mt-1 px-3 py-2 text-sm border rounded-md bg-background min-h-[100px] font-mono"
              placeholder="npm install&#10;cp .env.example .env"
            />
            <p className="text-xs text-muted-foreground mt-1">
              These commands run in the worktree after it's created
            </p>
          </div>
          <Button onClick={handleSaveStartCommands} disabled={saveStartCommandsMutation.isPending}>
            {saveStartCommandsMutation.isPending ? "Saving..." : "Save Setup Commands"}
          </Button>
        </div>
      </section>
      ...
      <section>
        <h3 className="text-sm font-medium mb-3">Terminal Start Command</h3>
        <div className="space-y-3">
          <div>
            <label className="text-sm text-muted-foreground">
              Command to run when opening terminal
            </label>
            <input
              type="text"
              value={terminalStartCommand}
              onChange={(e) => setTerminalStartCommand(e.target.value)}
              className="w-full mt-1 px-3 py-2 text-sm border rounded-md bg-background font-mono"
              placeholder="ls -la"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Runs in the persistent shell session after the prompt is ready
            </p>
          </div>
          <Button onClick={handleSaveStartCommands} disabled={saveStartCommandsMutation.isPending}>
            {saveStartCommandsMutation.isPending ? "Saving..." : "Save Terminal Command"}
          </Button>
        </div>
      </section>
    </div>
