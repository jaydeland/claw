...
export function AgentsSidebar({
  ...
  const handleDraftSelect = useCallback(
    ...
    [setSelectedChatId, setSelectedDraftId, setSelectedCategory, setSelectedMcpCategory, isMobileFullscreen, onChatSelect],
  )
  ...
  }, [filteredChats, selectedChatId, selectedChatIds, toggleChatSelection, setSelectedChatIds, setSelectedChatId, setSelectedMcpCategory, isMobileFullscreen, onChatSelect])
  ...
  // Checks for worktree, shows confirmation dialog if needed
  const handleArchiveSingle = useCallback(async (chatId: string) => {
    const worktreeStatus = await utils.chats.getWorktreeStatus.fetch({ chatId })

    if (worktreeStatus.hasWorktree) {
  ...
  }, [archiveChatMutation, utils.chats.getWorktreeStatus])
