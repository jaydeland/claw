...
export function DevServerPreview({
  ...
  // Terminal feature removed - no auto port detection or terminal control
  const detectedPorts: { port: number; address: string }[] = []
  ...
  const hasStartCommands = false

  // Handle play button click - open dev server settings
  const handlePlayClick = () => {
    setSettingsDialogOpen(true)
    setSettingsActiveTab("devserver")
    toast.info("Configure dev server", {
      description: "Terminal feature removed - enter URL manually below",
    })
  }
