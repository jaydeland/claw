...
const ONECODE_CONFIG_PATH = ".claw/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .claw/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  if (await fileExists(onecodePath)) {
    ...
    if (config) {
      return { config, path: onecodePath, source: "claw" }
    }
  }
...
export async function saveWorktreeConfig(
  ...
  target: "cursor" | "claw" | string = "claw",
